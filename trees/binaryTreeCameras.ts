...
export function minCameraCover(root: TreeNode | null): number {
  function depthFirstSearch(node: TreeNode | null): number[] {
    ...
    const [leftMinWithCamera, leftMinWithoutCamera, leftCoveredByParent] =
      depthFirstSearch(node.left);
    const [rightMinWithCamera, rightMinWithoutCamera, rightCoveredByParent] =
      depthFirstSearch(node.right);

    const minWithCamera =
      1 +
      Math.min(leftMinWithCamera, leftMinWithoutCamera, leftCoveredByParent) +
      Math.min(rightMinWithCamera, rightMinWithoutCamera, rightCoveredByParent);

    const minWithoutCamera = Math.min(
      leftMinWithCamera + rightMinWithCamera,
